...
<script>{`
  // The imported URL from Vite (includes the hashed asset name in prod)
  const spriteUrl = '${bootstrapIconsSprite}';
  ...
  function createIcon(iconId) {
    ...
    // Use the imported Vite URL + fragment identifier
  ...
  function randomToast() {
    ...
    document.querySelector('corn-toast').addToast({
      type: randomType,
...
The default toast component can be used to display simple messages to the user. It can be triggered by calling the `addToast` method on the toast component instance, passing in the message to be displayed and an optional duration for how long the toast should be visible.
...
<button class="corn-button" onclick="randomToast()" type="button">Add Toast</button>
